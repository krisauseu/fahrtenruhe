...
export function checkRuntimeEnv(): EnvCheck {
  ...
  if (!appUrl.trim()) {
    warnings.push(
      "APP_URL ist leer — Session Secure-Flag und Links können falsch sein.",
    );
  } else if (
...
export function logEnvCheckAtStartup(): void {
  ...
  for (const w of check.warnings) {
    console.warn(`[fahrtenruhe] ENV-Warnung: ${w}`);
  }
  for (const e of check.errors) {
    console.error(`[fahrtenruhe] ENV-Fehler: ${e}`);
  }
  if (!check.ok) {
    console.error(
      "[fahrtenruhe] Pflicht-ENV unvollständig — Login/Setup können fehlschlagen. Siehe .env.example.",
    );
  }
}
